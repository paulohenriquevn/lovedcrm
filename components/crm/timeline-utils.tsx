...
export function EntryContentRenderer({ entry }: { entry: TimelineEntry }): React.ReactElement {
  const isCommunication = entry.type === 'whatsapp' || entry.type === 'email' || entry.type === 'voip' || entry.type === 'note'
  ...
  return <div className="space-y-1">
    {entries.map((entry) => (
      <TimelineEntryComponent
        ...
        entry={entry}
        showLeadContext={showLeadContext}
        onClick={onEntryClick === null || onEntryClick === undefined ? undefined : () => onEntryClick(entry)}
      />
    ))}
  </div>
}
