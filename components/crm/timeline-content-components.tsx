...
export function CommunicationEntryContent({
  entry,
}: {
...
export function AISummaryEntryContent({ entry }: { entry: AISummaryEntry }): React.ReactElement {
  return (
    <div className="ml-8">
      <AISummaryCompact summary={entry.summary} confidence={entry.confidence} />
    </div>
  )
}
    ...
    <div className="ml-8">
      <div className="p-3 rounded-lg bg-orange-50 border border-orange-200">
        ...
        <div className="mt-2 text-xs text-gray-600 flex items-center gap-2">
          ...
          <code className="px-1 py-0.5 bg-white rounded text-red-600">{entry.oldValue}</code>
          <span>→</span>
          <code className="px-1 py-0.5 bg-white rounded text-green-600">{entry.newValue}</code>
        </div>
      </div>
    </div>
  )
...
function MeetingDescription({ description }: { description?: string }): React.ReactElement | null {
  ...
  return <p className="text-sm text-gray-700 mb-2">{description}</p>
}
...
function MeetingMetadata({ entry }: { entry: MeetingEntry }): React.ReactElement | null {
  ...
  const hasOutcome =
    entry.outcome !== null && entry.outcome !== undefined && entry.outcome.length > 0
  ...
  return (
    <div className="ml-8">
      <div className="p-3 rounded-lg bg-indigo-50 border border-indigo-200">
        <h4 className="text-sm font-medium text-gray-900 mb-1">{entry.title}</h4>
