...
function NoOrganizationState({ className }: { className: string }): JSX.Element {
  return (
    <div
      className={`flex items-center justify-between p-4 bg-background border-b border-destructive ${className}`}
    >
      <div className="flex items-center space-x-3">
  ...
  return (
    <div className="flex items-center space-x-3">
      ...
      <div className="min-w-0">
        <div className="flex items-center space-x-2">
          <h2 className="font-semibold text-lg truncate">{organization.name}</h2>
          {Boolean(organization.plan) && (
  ...
  return (
    <div className={`flex items-center space-x-2 ${className}`}>
      <CurrentOrganizationBadge
        ...
        className="h-6 w-6"
      />
      <span className="text-sm font-medium truncate max-w-32">{organization.name}</span>
      {Boolean(user?.role) && (
...
export function OrganizationHeaderWithActions({
  ...
  return (
    <div className={`flex items-center justify-between p-4 bg-background border-b ${className}`}>
      ...
      {Boolean(actions) && <div className="flex items-center space-x-2">{actions}</div>}
    </div>
  )
}
