...
export function IntegrationItem({ integration }: IntegrationItemProps): React.ReactElement {
    ...
    <div className="group">
      <div className="h-14 w-14 mx-auto bg-card border border-border rounded-lg flex items-center justify-center shadow-sm group-hover:scale-110 group-hover:shadow-md group-hover:border-primary/20 transition-all duration-300">
        {logoConfig !== undefined && logoConfig !== null ? (
          ...
          <div
            className={`h-10 w-10 ${integration.color} rounded flex items-center justify-center text-white font-bold text-xs`}
          >
            {integration.name.charAt(0)}
          </div>
        )}
      </div>
      <p className="text-xs text-muted-foreground mt-2 text-center">{integration.name}</p>
    </div>
  )
}
