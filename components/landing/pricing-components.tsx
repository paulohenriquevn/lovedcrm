    ...
    <CardHeader className="text-center pb-4">
      ...
      <div className="mt-4">
        ...
        {Boolean(isYearly && plan.yearlyDiscount) && (
          <p className="text-sm text-emerald-600 font-medium mt-2">{plan.yearlyDiscount}</p>
        )}

        {Boolean(plan.billingNote) && (
          <p className={`${TEXT_XS_MUTED_FOREGROUND_CLASS} mt-1`}>{plan.billingNote}</p>
        )}
      </div>
    </CardHeader>
  )
...
export function PricingCard({ plan, isYearly }: PricingCardProps): JSX.Element {
  ...
  return (
    ...
    >
      ...
      <Card
        className={`h-full ${plan.color} transition-all duration-300 ${plan.popular === true ? 'ring-2 ring-primary/20 shadow-xl' : 'hover:shadow-lg'}`}
      >
        ...
        <CardContent>
          ...
          <div className="space-y-3">
            <h4
              className={`font-semibold text-sm uppercase tracking-wide ${TEXT_MUTED_FOREGROUND_CLASS}`}
            >
  ...
  return (
    <div className="text-center mb-16">
      ...
      <p className={`text-xl ${TEXT_MUTED_FOREGROUND_CLASS} mb-8 max-w-3xl mx-auto`}>
        Escolha o plano ideal para sua agência. Sem compromisso de longo prazo, cancele quando
        quiser. Teste grátis por 14 dias.
      </p>

      <div className="flex items-center justify-center gap-4 mb-2">
        <span
          className={`text-sm ${isYearly ? TEXT_MUTED_FOREGROUND_CLASS : 'font-semibold text-foreground'}`}
        >
          Mensal
        </span>
        ...
        <span
          className={`text-sm ${isYearly ? 'font-semibold text-foreground' : TEXT_MUTED_FOREGROUND_CLASS}`}
        >
          Anual
        </span>
        <Badge className="bg-emerald-500/10 text-emerald-700 border-emerald-500/20">-20%</Badge>
      </div>

      <p className="text-xs text-muted-foreground">Economize 20% pagando anualmente</p>
    </div>
  )
}
    ...
    <div className="mt-16 text-center">
      ...
      <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto">
        ...
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground">Configure sua conta rapidamente</p>
        </div>

        <div className="text-center">
          <Crown className="h-8 w-8 text-purple-500 mx-auto mb-3" />
          <h4 className="font-semibold mb-2">Suporte premium</h4>
          <p className="text-sm text-muted-foreground">Atendimento especializado em português</p>
        </div>
      </div>
    </div>
  )
