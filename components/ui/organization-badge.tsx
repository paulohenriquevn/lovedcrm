...
export function OrganizationBadge({
  ...
  const tierKey =
    `tier${tier.charAt(0).toUpperCase() + tier.slice(1)}` as keyof typeof organizationTierLabels
  const displayContent =
    children ?? (showLabel === true ? organizationTierLabels[tierKey] : tierIcons[tier])
