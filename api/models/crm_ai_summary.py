...
class AISummary(Base):
    ...
    conversation_id: UUID = Column(SA_UUID(as_uuid=True), nullable=False, index=True)

    # Lead association (optional - summaries can be conversation-wide)
    lead_id: Optional[UUID] = Column(
        SA_UUID(as_uuid=True), ForeignKey("leads.id", ondelete="CASCADE"), nullable=True, index=True
    )
    ...
    def __repr__(self):
        """Return string representation of AISummary."""
