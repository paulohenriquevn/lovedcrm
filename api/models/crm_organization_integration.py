...
class OrganizationIntegration(Base):
    ...
    integration_metadata: Dict[str, Any] = Column(JSONB, nullable=False, default=dict)
    ...
    def create_email_integration(
        ...
        return cls(
            organization_id=organization_id,
            provider=IntegrationProvider.GMAIL if provider == 'gmail' else IntegrationProvider.OUTLOOK,
            encrypted_credentials=encrypted_credentials,
            status=IntegrationStatus.PENDING,
            metadata=metadata or {}
        )
    ...
    def create_voip_integration(
        ...
        return cls(
            organization_id=organization_id,
            provider=IntegrationProvider.TWILIO if provider == 'twilio' else IntegrationProvider.VOIP_PROVIDER,
            encrypted_credentials=encrypted_credentials,
            webhook_secret=webhook_secret,
            status=IntegrationStatus.PENDING,
            metadata=metadata or {}
