...
class UserTwoFactorService:
    ...
    def generate_qr_code(
        ...
        secret_key: str,
        user_email: str,
        organization_name: str = "SaaS Starter"
    ) -> str:
    ...
    def verify_2fa_for_login(
        ...
        if verified:
            self.repository.update_last_used(
                user_id=user.id,
                organization_id=organization.id
            )
    ...
    def cleanup_incomplete_setups(self, days_old: int = 7) -> int:
        ...
        incomplete_setups = self.db.query(UserTwoFactor).filter(
