...
class UserPreferencesService:
    ...
    def get_user_preferences(
        ...
        preferences = self.repository.get_by_user_and_organization(user.id, organization.id)
    ...
    def create_preferences(
        self, user: User, organization: Organization, preferences_data: UserPreferencesCreate
    ) -> UserPreferencesResponse:
        """Create new user preferences."""
        # Check if preferences already exist
        existing = self.repository.get_by_user_and_organization(user.id, organization.id)
    ...
    def update_preferences(
        self, user: User, organization: Organization, preferences_data: UserPreferencesUpdate
    ) -> UserPreferencesResponse:
        ...
        preferences = self.repository.create_or_update_preferences(
            user_id=user.id, organization_id=organization.id, preferences_data=update_data
        )
    ...
    def update_notifications(
        self, user: User, organization: Organization, notification_data: Dict[str, bool]
    ) -> UserPreferencesResponse:
    ...
    def update_display_preferences(
        self, user: User, organization: Organization, display_data: Dict[str, str]
    ) -> UserPreferencesResponse:
        """Update only display preferences."""
        preferences = self.repository.update_display(user.id, organization.id, display_data)
    ...
    def update_privacy_settings(
        self, user: User, organization: Organization, privacy_data: Dict
    ) -> UserPreferencesResponse:
    ...
    def update_quiet_hours(
        self, user: User, organization: Organization, quiet_hours_data: Dict
    ) -> UserPreferencesResponse:
        """Update quiet hours settings."""
        preferences = self.repository.update_quiet_hours(user.id, organization.id, quiet_hours_data)
    ...
    def quick_update(
        self, user: User, organization: Organization, updates: Dict
    ) -> UserPreferencesResponse:
        """Quick update for common preferences (theme, language, notifications)."""
        return self.update_preferences(user, organization, UserPreferencesUpdate(**updates))

    def reset_to_defaults(self, user: User, organization: Organization) -> UserPreferencesResponse:
        """Reset user preferences to default values."""
        preferences = self.repository.reset_to_defaults(user.id, organization.id)
    ...
    def delete_preferences(self, user: User, organization: Organization) -> bool:
    ...
    def get_organization_statistics(self, organization: Organization) -> UserPreferencesStatistics:
    ...
    def should_send_notification(
        ...
        if not preferences:
            ...
            return notification_type in ["email_security_alerts", "email_billing_alerts"]

        return preferences.should_send_notification(notification_type)

    def get_effective_settings(self, user: User, organization: Organization) -> Dict[str, str]:
        """Get effective language and timezone settings with fallbacks."""
        preferences = self.repository.get_by_user_and_organization(user.id, organization.id)

        if not preferences:
            return {"language": user.language or "en", "timezone": user.timezone or "UTC"}
    ...
    def _create_default_preferences(
        ...
        default_data = {"language": user.language or "en", "timezone": user.timezone or "UTC"}

        return self.repository.create_or_update_preferences(
            user_id=user.id, organization_id=organization.id, preferences_data=default_data
        )

    def bulk_notification_update(
        self, organization: Organization, notification_type: str, enabled: bool
    ) -> int:
