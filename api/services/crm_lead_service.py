...
class CRMLeadService:
    ...
    async def create_lead(self, organization: Organization, lead_data: LeadCreate, user_id: Optional[UUID] = None) -> Lead:
            ...
            try:
                ...
                try:
                    await websocket_manager.broadcast_to_organization(
                        organization.id, event_message  # No exclude_user_id - everyone gets the event
                    )
                    logger.debug(f"Successfully broadcasted lead_created event to org {organization.id}")
    ...
    async def update_lead_stage(
        ...
        try:
            ...
            if stage_data.notes:
                if lead.notes:
                    lead.notes += f"\n[Stage Update] {stage_data.notes}"
                else:
                    lead.notes = f"[Stage Update] {stage_data.notes}"
            ...
            try:
                lead_dict = {
                    ...
                    "name": lead.name,
                    "email": lead.email,
                    "stage": stage_data.stage.value if hasattr(stage_data.stage, 'value') else stage_data.stage,
                ...
                try:
                    ...
                    logger.debug(f"Successfully broadcasted lead_stage_changed event to org {organization.id}")
                except Exception as broadcast_error:
