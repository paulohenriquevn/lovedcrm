...
class UserPreferencesRepository(SQLRepository[UserPreferences]):
    ...
    def create_or_update_preferences(
        self, user_id: uuid.UUID, organization_id: uuid.UUID, preferences_data: Dict
    ) -> UserPreferences:
        ...
        if existing:
            ...
            preferences = UserPreferences(
                user_id=user_id, organization_id=organization_id, **preferences_data
            )
    ...
    def update_display(
        self, user_id: uuid.UUID, organization_id: uuid.UUID, display_preferences: Dict[str, str]
    ) -> Optional[UserPreferences]:
    ...
    def update_privacy_settings(
        self, user_id: uuid.UUID, organization_id: uuid.UUID, privacy_data: Dict
    ) -> Optional[UserPreferences]:
    ...
    def update_quiet_hours(
        self, user_id: uuid.UUID, organization_id: uuid.UUID, quiet_hours_data: Dict
    ) -> Optional[UserPreferences]:
    ...
    def update_custom_settings(
        self, user_id: uuid.UUID, organization_id: uuid.UUID, custom_settings: Dict
    ) -> Optional[UserPreferences]:
    ...
    def delete_preferences(self, user_id: uuid.UUID, organization_id: uuid.UUID) -> bool:
    ...
    def get_users_with_email_notifications_enabled(
        self, organization_id: uuid.UUID, notification_type: str = "email_notifications"
    ...
    def get_preferences_statistics(self, organization_id: uuid.UUID) -> Dict:
        ...
        theme_light = (
            self.session.query(UserPreferences)
            .filter(
                UserPreferences.organization_id == organization_id, UserPreferences.theme == "light"
            )
            .count()
        )
        ...
        theme_dark = (
            self.session.query(UserPreferences)
            .filter(
                UserPreferences.organization_id == organization_id, UserPreferences.theme == "dark"
            )
            .count()
        )
        ...
        most_common_language = (
            self.session.query(UserPreferences.language)
            .filter(UserPreferences.organization_id == organization_id)
        ...
        most_common_timezone = (
            self.session.query(UserPreferences.timezone)
            .filter(UserPreferences.organization_id == organization_id)
    ...
    def bulk_update_notification_preference(
        self, organization_id: uuid.UUID, notification_type: str, enabled: bool
    ) -> int:
