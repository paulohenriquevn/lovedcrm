...
from fastapi import APIRouter, WebSocket, WebSocketDisconnect, Query, HTTPException, status, Depends, Request
from fastapi.responses import JSONResponse

from api.core.websocket_manager import websocket_manager
from api.core.security import verify_token
from api.core.deps import get_db, get_current_organization, get_current_active_user, get_org_id_from_header
from api.models.organization import Organization
from api.models.user import User
from sqlalchemy.orm import Session

router = APIRouter(prefix="/api/ws", tags=["WebSocket - Real-time Collaboration"])
...
async def websocket_collaboration_endpoint(
    ...
    **Events Sent:**
    ...
    try:
        ...
        await websocket_manager.connect(websocket, UUID(str(organization.id)), UUID(str(user.id)), user_info)
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if any(keyword in error_msg for keyword in ["disconnect", "closed", "connection", "receive", "message has been received"]):
...
async def handle_client_message(websocket: WebSocket, user: User, organization: Organization, message: Dict[str, Any]):
